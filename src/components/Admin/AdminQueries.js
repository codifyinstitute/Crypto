...
const AdminQueries = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch('https://crypto-anl6.onrender.com/contacts/all');
      if (!response.ok) {
  ...
  const updateStatus = async(que) =>{
    // console.log(que)
    try {
        const response = await fetch(`https://crypto-anl6.onrender.com/contacts/update/${que._id}/status`, {
          method: 'PATCH',
          headers: {
